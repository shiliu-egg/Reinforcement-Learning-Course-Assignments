...
def main(model):
    ...
    for i in range(EPISODES if not TEST else 1):
        ...
        if not TEST:
            if ep_reward < -200 and done_cnt >= 10:
                memory.remove_last()
        ...
        print(
            f"Episode: {i}, Memory Size: {memory.size()}, Done cnt: {done_cnt}, Reward: {round(ep_reward, 3)}"
        )


if __name__ == "__main__":
    # hyper-parameters
    EPISODES = 300  # 训练/测试幕数
    BATCH_SIZE = 800
    LR = 0.00025
    GAMMA = 0.98
    ...
    SEED = 0
    MODEL_PATH = ""
    SAVE_PATH_PREFIX = os.path.join("log", "Mountaincar")
    TEST = True

    env = gym.make("MountainCar-v0", render_mode="rgb_array" if TEST else None)
